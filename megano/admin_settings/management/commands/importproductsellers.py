...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("seller_id", type=int, help="Введите id продавца. Обязательный позиционный аргумент.")
        parser.add_argument(
            "-e",
            "--email",
        ...
        if files:
            for file in files:
                ...
                try:
                    ...
                    Log.critical(
                        user=None, import_id=import_id, message=f'Файл "{file}" не соответствует формату JSON.'
                    )
                    Log.info(user=None, import_id=import_id, message=f'Файл "{file}" перемещен в {destination_path}.')
            if data_tuples:
                ...
                Log.critical(
                    user=None, import_id=import_id, message="Ни один из файлов не прошел валидацию. Импорт отменен."
                )
        else:
